...
def display_results(query_image_path, matches, database_path):
    ...
    if len(query_img.shape) == 2:
        ...
        query_img = (query_img * alpha[:,:,np.newaxis] + white_bg[:,:,np.newaxis] * (1 - alpha[:,:,np.newaxis])).astype(np.uint8)
    ...
    for i, (name, dist) in enumerate(matches[:10], start=1):
        ...
        if len(img.shape) == 2:
            ...
            img = (img * alpha[:,:,np.newaxis] + white_bg[:,:,np.newaxis] * (1 - alpha[:,:,np.newaxis])).astype(np.uint8)
        elif img.shape[-1] == 3:
...
if __name__ == "__main__":
    ...
    default_query = r"C:\Users\李晓璇\Desktop\Semester 4\CS6123\project_3\scikit-image\test_dataset\test23.jpg"
    # 获取命令行参数或使用默认值
